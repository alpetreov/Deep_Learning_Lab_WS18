...
class MyWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        validation_error, model = train_and_validate(self.x_train, self.y_train, self.x_valid, self.y_valid, epochs, lr, num_filters, batch_size, filter_size)
        # TODO: We minimize so make sure you return the validation error here
        return ({
            'loss': validation_error[-1],  # this is the a mandatory field to run hyperband
