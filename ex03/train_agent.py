...
def preprocessing(X_train, y_train, X_valid, y_valid, history_length=1):
    ...
    for i in range(y_train.shape[0]):
        y_new.append(action_to_id(y_train[i]))
    for i in range(y_valid.shape[0]):    
        y_new_valid.append(action_to_id(y_valid[i]))
    # cast the lists to np arrays
    y_new = np.array(y_new)
    y_new_valid = np.array(y_new_valid)
    # one hot encode the labels for easier classification
    y_train = one_hot(y_new)
    y_valid = one_hot(y_new_valid)
